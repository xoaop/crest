...
FunctionDecl::Linkage classify_linkage(bool is_extern_c, bool is_builtin, bool has_symbol, bool is_instance) {
    if(is_extern_c || is_builtin) {
        return FunctionDecl::Linkage::PureDeclaration;
    } else if(is_instance || !has_symbol) {
        return FunctionDecl::Linkage::MergableDefinition;
    } else {
        return FunctionDecl::Linkage::Definition;
    }
}
...
xpString mangle_name(Ref<CIRInstResult> key, bool is_extern_c, TypeRef func_type) {
    auto& map = name_cache();
